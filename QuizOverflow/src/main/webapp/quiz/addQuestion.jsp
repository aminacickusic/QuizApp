...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('next-question').addEventListener('click', function(event) {
        ...
        if (isAnyFieldEmpty()) {
            ...
            document.getElementById('question-phase').value = 'next-question';
            document.getElementById('answer-form').submit();
        }
    });
    
    
    if (document.getElementById('save-quiz')!=null) {
    document.getElementById('save-quiz').addEventListener('click', function(event) {
        event.preventDefault();
        const answerDiv = document.getElementById('answer-div');
        if (isAnyFieldEmpty()) {
        	event.preventDefault();
...
<body>
	<section class="ftco-section">
		...
		<div class="main-div" >
				...
				Integer ordNum = (Integer) request.getSession().getAttribute("ordinalNumber");
		      ...
		      <button class="color-button question-number-btn" style="background-color: <%=currentColor%>"><%=request.getSession().getAttribute("ordinalNumber")%></button>
				</div>
				<div class="quit-btn">
				...
				<br>
				<form action="<%=request.getContextPath()%>/admin/addquestion" id="answer-form" method="POST" style="width: 100%;">
				...
				<%
				Integer num = (Integer) request.getSession().getAttribute("ordinalNumber");
				if (num < 3) {
					
				
				%>
			    <p style="width: 30%; text-align: center;">Quiz must have at least 3 questions :) </p>
			    <%
				} else {
				  	
				
			    %>
			    <button class="pushable-button" id="save-quiz">Save quiz</button>
			    <br>
				...
				<button class="pushable-button" id="next-question">Next question</button>
